...
class ForumListSerializer(serializers.ModelSerializer):
    ...
    def get_last_activity(self, obj):
        try:
            ...
            post = (
                Post.objects.filter(thread__forum=obj).order_by("-created_at").first()
            )
...
class ForumDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Forum
        fields = ("slug", "name", "description", "threads")
        read_only_fields = ("slug",)
        lookup_field = "slug"
    ...
    def get_threads(self, obj):
        def get_last_activity(thread):
            try:
                post = (
                    Post.objects.filter(thread=thread).order_by("-created_at").first()
                )
                if post:
                    return {
                        "avatar": post.creator.profile.avatar,
        ...
        try:
            threads = Thread.objects.filter(forum=obj).order_by(
